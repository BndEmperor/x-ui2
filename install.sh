...
config_after_install() {
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        ...
        echo -e "${red}"${red} has been cancelled, all settings are default settings, please modify in time ${plain}"
    fi
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        echo -e "${yellow}Detected the latest version of x-ui: ${last_version}, start the installation ${plain}"
